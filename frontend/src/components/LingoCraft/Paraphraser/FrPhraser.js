...
import { paraphraseTextInFrench } from '../../../service/engPhraserService';
...
const FrPhraser = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await paraphraseTextInFrench(input, mode, sliderValue);
    ...
    <Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', marginLeft: '10px' }}>
        ...
        <div style={{
          ...
          <FormLabel style={{ color: '#0C2E42', border: '2px solid #0C2E42' }}>Synonymes</FormLabel>
          <OverlayTrigger
            placement="top"
            overlay={
              <Tooltip id="tooltip">
                Une valeur plus élevée signifie plus de changements dans le texte.
              </Tooltip>
            }
          >
            <Form.Range
        ...
        <Card style={{ flex: '1', marginRight: '10px', height: 'auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ marginTop: '10px' }}>{wordCountOut} mots.</p>
          </div>
        </Card>
      </div>
    </Card>
